<?php
/*
Plugin Name: Philly lost Widget Powered By Everyblock 
Plugin URI: http://tirespider.com/
Description: Add a Philly Lost and Found Widget to a post or page.
Version: 2.0.2
Author: Jeff S
...
class PhillylostWidget extends WP_Widget {

	public static $metro = "philly";
	public static $schema = "lost-found";
	
	public function PhillylostWidget()
	{
		$widget_ops = array('classname' => 'PhillylostWidget', 'description' => 'Embeds a Philly Lost and Found Widget' );
		$this->WP_Widget('PhillylostWidget', 'Philly Lost and Found Widget', $widget_ops);
	}
...
add_action('widgets_init', create_function('', 'return register_widget("PhillylostWidget");') );

function display_Philly_lost_widget($atts) {
	$atts = extract(shortcode_atts( array('width' => '300', 'height' => '500'), $atts, 'display_Philly_lost_widget'));
	
	$code = "<iframe src=\"" . plugins_url( 'widget.php', __FILE__ ) . "?metro=" . PhillylostWidget::$metro . "&schema=" . PhillylostWidget::$schema . "&width=" . $width . "&height=" . $height . "&url=" . ABSPATH . "\" width=\"". $width . "\" //height=\"" . $height . "\" frameborder='0' scrolling='no'></iframe>";
	 
	return get_html_for_embed_Philly_lost(stripslashes($code));
}

function get_html_for_embed_Philly_lost($embedCode) {
	return '<div class="widget_block">' . stripslashes($embedCode) .'</div>';  
}

add_shortcode('display_Philly_lost_widget', display_Philly_lost_widget);
